...
namespace GPS\AppBundle\Serializer;
...
class DateTimeHandler implements SubscribingHandlerInterface
{
    ...
    private function parseDateTime($data, array $type)
    {
        ...
        // There's a bug in `DateTime::createFromFormat` when dealing with
        // negative timestamps (at least I think that's issue), so this
        // checks explicitly for timestamps and handles them separately
        if ('U' === $format) {
            $ts = (int) $data;
            $dt = new \DateTime();
            $dt->setTimestamp($ts);
